...
# timestep_input -> EM "TIME STEP [QUERY]"  (already normalized before feeding)
...
goal_to_hidden = MappingProjection(
    ...
    matrix=(0.1 * np.random.rand(obs_len, 20) - 0.1),  # Range: [-0.1, 0.1]
    learnable=True
)
hidden_to_output = MappingProjection(
    sender=hidden_layer,
    receiver=mlp_output,
    matrix=(0.1 * np.random.rand(20, 7) - 0.1),  # Range: [-0.1, 0.1]
    learnable=True
)
    ...
    for trial in range(num_trials):
        ...
        while True:
            ...
            if len(successful_episodes) == 1:
                print("\n" + "=" * 70)
                print("  FIRST SUCCESS ACHIEVED! Switching to EM...")
